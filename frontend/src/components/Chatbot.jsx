...
const Chatbot = () => {
      ...
      setMessages([
        {
          id: 1,
          type: "bot",
          text: "Hello! Welcome to our FAQ. You can search for common questions below or type a unique question to contact us.",
        },
      ]);
    }
  }, [isOpen, hasOpened]);
  ...
  const handleQuestionClick = (questionText, replyText) => {
    ...
    setMessages((prev) => [
      ...prev,
      { id: Date.now(), type: "user", text: questionText },
    ]);
    setIsTyping(true);
    setTimeout(() => {
      setIsTyping(false);
  ...
  const handleSendMessage = (e) => {
    ...
    setInputValue("");
    setIsOpen(false);
    navigate("/contact-us", { state: { message: userText } });
  };
  ...
  const filteredFaqs = faqs.filter(
    (q) =>
      ...
      q.answer.toLowerCase().includes(inputValue.toLowerCase()),
  );
  ...
  const themeStyles = {
    ...
    botMessage:
      actualTheme === "dark"
        ? "bg-gray-700 border-gray-600 text-gray-100 shadow-none"
    ...
    optionBtn:
      actualTheme === "dark"
        ? "bg-gray-800 text-blue-400 border-gray-700 hover:bg-gray-700 hover:text-blue-300"
        : "bg-blue-50 text-blue-700 border-blue-100 hover:bg-blue-100 hover:text-blue-800",
    inputArea:
      actualTheme === "dark"
        ? "bg-gray-900 border-gray-700"
    ...
    inputField:
      actualTheme === "dark"
        ? "bg-gray-800 border-gray-700 text-gray-100 focus:ring-blue-500 placeholder-gray-500"
  ...
  return (
    <div className="fixed bottom-4 right-4 sm:bottom-6 sm:right-6 z-[9999] font-sans">
      {isOpen && (
        <div
          className={`mb-4 w-[calc(100vw-2rem)] sm:w-96 max-h-[85vh] rounded-2xl shadow-2xl border flex flex-col overflow-hidden transition-colors duration-300 transform origin-bottom-right ${themeStyles.container}`}
        >
          {/* Header */}
          <div
            className={`p-3 sm:p-4 flex justify-between items-center drop-shadow-md shrink-0 transition-colors duration-300 ${themeStyles.header}`}
          >
              ...
              <div className="relative">
                ...
                {isDropdownOpen && (
                  <>
                    ...
                    ></div>
                    <div
                      className={`absolute right-0 top-10 w-32 rounded-lg py-1 shadow-lg border z-20 ${themeStyles.dropdownMenu}`}
                    >
                      <button
          ...
          {/* Messages Area */}
          <div
            className={`flex-1 p-3 sm:p-4 min-h-[200px] overflow-y-auto flex flex-col space-y-3 sm:space-y-4 transition-colors duration-300 ${themeStyles.messagesArea}`}
          >
            {messages.map((msg) => (
              <div
                key={msg.id}
                className={`max-w-[85%] rounded-2xl p-2.5 sm:p-3 text-sm ${
                  msg.type === "user"
                    ? themeStyles.userMessage + " self-end rounded-br-sm"
                    : themeStyles.botMessage +
                      " self-start rounded-bl-sm border"
            ...
            {isTyping && (
              <div
                className={`${themeStyles.botMessage} rounded-2xl p-3 w-16 self-start rounded-bl-sm flex space-x-1 items-center justify-center border transition-colors`}
              >
                <div
                  className={`w-2 h-2 rounded-full animate-bounce ${themeStyles.typingDot}`}
          ...
          {/* FAQ Topics (Searchable) */}
          <div
            className={`p-2 sm:p-3 border-t max-h-32 overflow-y-auto shrink-0 transition-colors duration-300 ${themeStyles.optionsArea}`}
          >
            <p
              className={`text-[10px] sm:text-xs mb-1.5 sm:mb-2 px-1 ${themeStyles.optionText}`}
            >
              {filteredFaqs.length > 0
                ? "Related FAQs:"
                : "No matching FAQs found. You can send your unique question below."}
            </p>
            <div className="flex flex-wrap gap-1.5 sm:gap-2">
              {filteredFaqs.map((q) => (
                <button
                  ...
                  title={q.question}
                >
                  {q.question.length > 35
                    ? q.question.substring(0, 35) + "..."
                    : q.question}
                </button>
              ))}
            </div>
          ...
          <form
            ...
            className={`p-2 sm:p-3 border-t flex items-center gap-2 shrink-0 transition-colors duration-300 ${themeStyles.inputArea}`}
          >
            <input
              type="text"
              value={inputValue}
              onChange={(e) => setInputValue(e.target.value)}
              placeholder="Search FAQ or type inquiry..."
            ...
            <button
              type="submit"
              title="Send inquiry to Contact Us"
            ...
            >
              <Send
                size={16}
                className="translate-x-[1px] sm:w-[18px] sm:h-[18px]"
              />
            </button>
          </form>
        </div>
